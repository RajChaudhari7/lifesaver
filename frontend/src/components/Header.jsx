...
const Header = () => {
    ...
    return (
        <div ref={containerRef} className='relative flex flex-col md:flex-row flex-wrap bg-gradient-to-br from-primary to-blue-700 rounded-2xl px-6 md:px-10 lg:px-20 overflow-hidden shadow-2xl'>
            ...
            {/* --- Decorative Floating Elements (GSAP 3D Targets) --- */}
            <div 
                ref={addToFloatingRefs} 
                className='absolute top-[10%] left-[5%] w-12 h-12 bg-white/10 backdrop-blur-md rounded-full shadow-lg border border-white/20 z-0'
            ></div>
            <div 
                ref={addToFloatingRefs} 
                className='absolute bottom-[20%] left-[45%] w-24 h-24 bg-blue-400/20 backdrop-blur-md rounded-full shadow-lg border border-white/10 z-0'
            ></div>
            <div 
                ref={addToFloatingRefs} 
                className='absolute top-[30%] right-[10%] w-8 h-8 bg-white/15 backdrop-blur-sm rounded-md shadow-lg border border-white/20 transform rotate-45 z-0'
            ></div>
            ...
            <div className='relative z-10 md:w-1/2 flex justify-center items-end mt-8 md:mt-0'>
                <img 
                    ref={imageRef}
                    className='w-full max-w-lg md:absolute bottom-0 h-auto rounded-lg drop-shadow-2xl object-cover' 
